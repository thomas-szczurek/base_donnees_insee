...
CREATE MATERIALIZED VIEW local.recur_appartenance_zonages AS
WITH RECURSIVE st(fk_zonage_enfant, fk_zonage_parent) AS (
  ...
  FROM local.appartenance_zonages AS a
	JOIN local.zonages_administratifs AS n ON n.pk_id = a.fk_zonage_enfant
  WHERE fk_type = 1
  UNION ALL
  SELECT 
		a.fk_zonage_enfant, 
...
FROM st
	JOIN local.zonages_administratifs AS p ON p.pk_id = st.fk_zonage_parent
